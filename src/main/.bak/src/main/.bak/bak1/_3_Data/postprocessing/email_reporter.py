...
class EmailReporter:
	...
	def split_files_by_category(self, source_dir: str, max_size_mb: int = 25) -> Tuple[List[List[Tuple[str, str]]], List[List[Tuple[str, str]]]]:
		...
		print(f"📊 HTML files: {len(html_files)} files, {html_total_size / (1024 * 1024):.1f} MB")
		print(f"📊 Other files: {len(other_files)} files, {other_total_size / (1024 * 1024):.1f} MB")
	...
	def _split_file_group(self, files: List[Tuple[str, str]], max_size_bytes: int, group_name: str) -> List[List[Tuple[str, str]]]:
		...
		print(f"📦 Split {group_name} into {num_parts} part(s)")
		for i, group in enumerate(file_groups):
			group_size = sum(os.path.getsize(fp) for fp, _ in group)
			print(f"  Part {i+1}: {len(group)} files, {group_size / (1024 * 1024):.1f} MB")
	...
	def create_category_archives(self, source_dir: str, base_archive_name: str, 
		...
		for i, file_group in enumerate(html_file_groups, 1):
			...
			print(f"✅ Created HTML archive: {archive_name} ({archive_size:.1f} MB)")
		...
		for i, file_group in enumerate(other_file_groups, 1):
			...
			archive_paths.append(archive_path)
			archive_size = os.path.getsize(archive_path) / (1024 * 1024)
			print(f"✅ Created other files archive: {archive_name} ({archive_size:.1f} MB)")
	...
	def format_statistics_table(self, df: pd.DataFrame) -> str:
		...
		def format_number(x):
			...
			# Если это строка, попробуем преобразовать в число
			if isinstance(x, str):
				# Убираем запятые в больших числах
				x_clean = x.replace(',', '').replace(' ', '')
				try:
					# Пробуем преобразовать в float
					x_num = float(x_clean)
					# Проверяем, было ли исходное значение процентным
					if '%' in x:
			...
			if isinstance(x, (int, float)):
				...
				if x == int(x):
					return f"{int(x)}"  # Убираем .0 для целых чисел
				else:
		...
		for col in formatted_df.columns:
			# Применяем форматирование ко всем ячейкам столбца
			if col.lower() != 'sample_id':
				formatted_df[col] = formatted_df[col].apply(format_number)

				# Дополнительная обработка для процентных столбцов
				if any(keyword in col.lower() for keyword in ['fraction', 'percent', '%', 'percentage']):
				...
				if any('%' in word for word in formatted_df[col].to_list()):
					formatted_df[col]	=	formatted_df[col].apply(lambda x: str(x).replace('.', ','))
		# Финальная обработка для замены "N/A" на пустые строки
...
def archive_and_send_report(sum_path: str, flowcell: str, ceph_paths: List[str],
	...
	try:
		...
		print(f"🕒 Creating single archive to check size: {single_archive_name}")
		...
		print(f"📊 Total archive size: {archive_size_mb:.1f} MB (limit: {max_archive_size_mb} MB)")
		...
		if archive_size_mb <= max_archive_size_mb:
			...
			print("⚠️ Archive exceeds size limit, splitting HTML files separately...")
		...
		print(f"📦 Found {len(archive_paths)} archive(s) to send")
		...
		for part_number, archive_path in enumerate(archive_paths, 1):
			...
			print(f"📦 Processing archive {part_number}/{len(archive_paths)}: {archive_name} ({part_size_mb:.1f} MB)")
			...
			# Send email
			print(f"🕒 Sending email {part_number}/{len(archive_paths)}...")
			...
			if success:
				success_count += 1
				print(f"✅ Successfully sent archive {part_number}/{len(archive_paths)}")
		...
		if success_count == len(archive_paths):
			print(f"✅ All {len(archive_paths)} email(s) sent successfully!")
			return True
		else:
			print(f"⚠️ Only {success_count} out of {len(archive_paths)} email(s) were sent successfully")
			return False
