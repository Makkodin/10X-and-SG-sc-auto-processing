...
def check_and_move_reports(
	...
	samples_list    =   samples_parse_df['Sample_ID'].unique()
	df_path_samples	=	[]
	for sample_name in samples_list:
		df_sample   =   samples_parse_df[samples_parse_df['Sample_ID'] == sample_name]
		
		if len(df_sample) != 1:
			print(f'❌[Check & move] Sample name is not unique!')
			break
		_sample     =   sample_name
		_seq_type   =   df_sample['SEQtype'].unique().tolist()[0]
		...
		if 'VDJ_type' in df_sample.columns and not pd.isna(df_sample['VDJ_type'].iloc[0]):
			_seq_type		=	"SC_SeekGene_VDJ"
			_table_seqtype  =   "SC_SeekGene_RNA|SC_SeekGene_VDJ"
			df_sample.loc[:, 'SEQtype'] = _table_seqtype
		...
		report_path =   glob(f'{result_dir}/{flowcell[0]}/{_sample}*/*{_postfix_report}')
		stat_path   =   glob(f'{result_dir}/{flowcell[0]}/{_sample}*/*{_postfix_stats}')

		if report_path:
			report_path		=	report_path[0]
			stat_path		=	stat_path[0] if stat_path else "Error"
			if _table_seqtype:
				df_path_samples.append([_sample, 		_table_seqtype, 
										report_path, 	stat_path,
										result_dir, 	ceph_res_dir])
			else:
				df_path_samples.append([_sample, 		_seq_type, 
										report_path, 	stat_path,
										result_dir, 	ceph_res_dir])
	update_path_df			=	pd.DataFrame(df_path_samples)
	update_path_df.columns	=	['Sample_ID', 'SEQtype', 'Report_path','Stat_path','Local_path','Ceph_path']	
	if 'VDJ_type' in samples_parse_df.columns:
		mask = samples_parse_df['VDJ_type'].notna()
		if _table_seqtype:
			samples_parse_df.loc[mask, 'SEQtype'] = _table_seqtype
		samples_parse_df		=	pd.merge(samples_parse_df, update_path_df, how='left', on=['Sample_ID', 'SEQtype'])
	else:
		samples_parse_df		=	pd.merge(samples_parse_df, update_path_df, how='left', on=['Sample_ID', 'SEQtype'])
	print(samples_parse_df)
	...
	try:
		...
		elif type(fastq_res_folder) == list:
			remove_com = ['sshpass', '-p', password, 'sudo', 'rm', '-rf', runsheet_path]
			subprocess.run(remove_com, check=True, stdout=subprocess.PIPE, stderr=subprocess.PIPE)
			for f_fastq in fastq_res_folder:
				remove_com = ['sshpass', '-p', password, 'sudo', 'rm', '-rf', f_fastq]
				subprocess.run(remove_com, check=True, stdout=subprocess.PIPE, stderr=subprocess.PIPE)
			print("✅[Check & move] All FASTQ files and temporary files removed!")
	except subprocess.CalledProcessError as e:
