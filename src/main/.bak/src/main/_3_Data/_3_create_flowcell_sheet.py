...
def combine_subtype(group):
    # Сортируем значения: сначала RNA, потом ATAC
    sorted_subtypes = sorted(group['SubType'].unique(), 
                            key=lambda x: ['RNA', 'ATAC'].index(x) if x in ['RNA', 'ATAC'] else 2)
    return '-'.join(sorted_subtypes)

def create_run_sheet(fastq_save:Union[str, list],
                     infosheet:pd.DataFrame,
                     runsheet_save:str,
                     img_save:str,
                     flowcell:Union[str, list],
                     supported_type:list
    ...
    _path_to_process        =   {'SC_TENX_RNA'           :   'src/main/_2_Commands/10X/_10X_scRNA.py',
                                 ...
                                 'SC_TENX_Multiome'      :   'src/main/_2_Commands/10X/_10X_scMultiome.py',
                                 ...
                                 'SC_SeekGene_Multiome'  :   'src/main/_2_Commands/SG/_SG_scMultiome.py',
                                 }

    # SKIP SEQtype  

    skip_types              =   ['SC_TENX_Visium_FF']
    ...
    for sample_infosheet_support in samples_infosheet_support:
        if type(fastq_save) == list:
            for f_s in fastq_save:
                samples.extend(list(set([x.split('/')[-1].split('_S')[0] for x in glob(f"{f_s}/{sample_infosheet_support}_S*.gz")])))
        else:
    ...
    if len(samples) == len(flowcell_parse_df):
        ...
        seq_types                           =   samples_parse_df['SEQtype'].unique().tolist()
        ...
        if any('Multiome' in item for item in seq_types):
            multiome_mask = samples_parse_df['SEQtype'].str.contains('Multiome')

            samples_parse_df.loc[multiome_mask, 'SubType'] = samples_parse_df.loc[multiome_mask, 'SEQtype'].apply(
                lambda x: x.rsplit('_', maxsplit=1)[1]
            )
            samples_parse_df.loc[multiome_mask, 'SEQtype'] = samples_parse_df.loc[multiome_mask, 'SEQtype'].apply(
                lambda x: x.rsplit('_', maxsplit=1)[0]
            )

            pattern = r'\d{6}_.*-\d{6}_.*'

            multiome_flowcell = next((item for item in flowcell if isinstance(item, str) and re.match(pattern, item)), None)
            samples_parse_df.loc[multiome_mask, 'Flowcell'] = multiome_flowcell
            descr = '-'.join(samples_parse_df.loc[multiome_mask, 'Description'].unique())
            tissue = '-'.join(samples_parse_df.loc[multiome_mask, 'Tissue'].unique())

            multiome_df = samples_parse_df[multiome_mask].copy()
            multiome_grouped = multiome_df.groupby(['Flowcell', 'Sample_ID', 'Reference', 
                                                   'Check_ORG', 'SEQtype', 'Organism_name']).apply(
                lambda x: pd.Series({'SubType': combine_subtype(x)})
            ).reset_index()

            # Добавляем Description и Tissue
            multiome_grouped['Description'] = descr
            multiome_grouped['Tissue'] = tissue
            samples_parse_df = pd.concat([
                samples_parse_df[~multiome_mask],  # Не-multiome строки
                multiome_grouped  # Обработанные multiome строки
            ], ignore_index=True)
            flowcell    = next((item for item in flowcell if isinstance(item, str) and re.match(pattern, item)), None)
        else:
            flowcell    =   flowcell[0]
        ...
        samples_parse_df            =   samples_parse_df.drop(columns='Check_ORG').sort_values('Sample_ID')
        samples_parse_df            =   samples_parse_df.reset_index(drop='index')
        samples_parse_df.index      +=  1  

        pattern     = r'\d{6}_.*-\d{6}_.*'
        
        sample_sheet_info_path      =   f"{runsheet_save}/{flowcell}-run_sheet.csv" 
        samples_parse_df.to_csv(sample_sheet_info_path, index=False)
        ...
        if 'SubType' in samples_parse_df.columns:
            print(tabulate(samples_parse_df[['Sample_ID','Flowcell' ,'Reference', 
                                             'SEQtype'  ,'SubType'  ,'Tissue']], 
