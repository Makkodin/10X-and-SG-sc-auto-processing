...
def process_sample(
    ...
    # Check if exist
    if 'grch38' in organism.lower():
        postfix_org = 'h' 
    elif 'mm10' in organism.lower():
        postfix_org = 'm'
    elif 'macmul' in organism.lower():
        postfix_org = 'mmul'

    if existing_results:
        log_file = open(f"{result_dir}/{flowcell}/{sample_id}_{postfix_org}_output.log", "w")
...
def processing_flowcell(
    ...
    """
    samples_num     =   len(runsheet['Sample_ID'].unique())
    flowcells       =   runsheet["Flowcell"].unique().tolist()
    organisms       =   runsheet["Reference"].unique().tolist()
    organisms_name  =   runsheet["Organism_name"].unique().tolist()
    typeseqs        =   runsheet["SEQtype"].unique().tolist()
    tissue          =   runsheet["Tissue"].unique().tolist()
    ...
    print(f"│ ℹ️ Organism reference___________:   {organisms_name}")
    ...
    VDJ_key                 =   False
    Multiome_key            =   False
    multiome_tenx_ss_path   =   'None'
    if 'SC_SeekGene_VDJ' in typeseqs:
        VDJ_key =   True
    elif any('Multiome' in item for item in typeseqs):
        Multiome_key            =   True
    ...
    for _, row in runsheet.iterrows():
        ...
        proc, log_file, ceph_path = process_sample(
            ...
            multiome_tenx_ss_path   =   multiome_tenx_ss_path
        )
        if proc != None:
