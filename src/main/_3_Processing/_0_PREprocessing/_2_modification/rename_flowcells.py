...
def process_cellplex_data(df:pd.DataFrame,
	...
	for flowcell in cellplex_df['Flowcell'].unique():
		...
		cellplex_df_flowcell[['CellPlex_ID', 'CMO', 'PLEX']]    =   cellplex_df_flowcell['Description'].apply(lambda x: extract_cellplex_data(x))
		cellplex_df_flowcell                                    =   merge_plex_rows_compact(df = cellplex_df_flowcell)
		flowcell_drop_result_df.append(flowcell)
		if len(cellplex_df_converted)   == 0:
			...
			cellplex_df_converted   =   pd.concat([cellplex_df_converted, 
												   ...
												   axis=0,
												   ignore_index=True)
	result_df   =   result_df[~result_df['Flowcell'].isin(flowcell_drop_result_df)]
