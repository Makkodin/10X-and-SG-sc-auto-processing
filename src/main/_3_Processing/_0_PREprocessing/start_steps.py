...
def fix_flowcell(x):
    ...
    Desct_TYPE  =   x['Desct_TYPE']

    if Desct_TYPE != 'SC_TENX_CellPlex' and 'plex=' in Description:
        Desct_TYPE  =   'SC_TENX_CellPlex'
    return Desct_TYPE
...
def update_info_sheet(  workdir:str,
    ...
    ######################################
    # Custom error
    idx = info_sheet[(info_sheet['Sample_ID'] == '770097261301') & 
                 (info_sheet['Flowcell'] == '211228_VH00195_63_AAAHVLNM5')].index
    info_sheet = info_sheet.drop(idx)
    ######################################

    info_sheet['Desct_TYPE']      =   info_sheet.apply(lambda x: fix_flowcell(x), axis=1)
